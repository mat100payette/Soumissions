...
{
    public partial class RapportDetail
    {
        ...
        partial void RapportDetail_InitializeDataWorkspace(List<IDataService> saveChangesTo)
        {
            if (!initDone)
            {
        ...
        partial void Rapport_Changed()
        {
            SetDisplayNameFromEntity(Rapport);
        }

        partial void RapportDetail_Saved()
        {
            SetDisplayNameFromEntity(Rapport);
        }

        partial void RapportDetail_Created()
        {
            
        }

        partial void Rapport_Loaded(bool succeeded)
        {
            SetDisplayNameFromEntity(Rapport);
        }
        ...
        partial void ProjetEtapesQuery_Loaded(bool succeeded)
        {
            if (vendeurs != null)
            {
                projetEtapes = ProjetEtapesQuery.Where(p => vendeurs.Contains(p.Projet.Vendeur)).ToList();
            }
        }
        ...
        partial void VendeurRapports_Loaded(bool succeeded)
        {
            vendeurs = VendeurRapports.Select(vr => vr.Vendeur).Distinct().ToList();
        }
        ...
        private double getTotalEtapeWeek(string etape, DateTime dateContenue, List<string> vendeurs)
        {
            double totalEtape = 0;

            totalEtape = Convert.ToDouble((from p in peData
                                           where p.Item2.Equals(etape) && vendeurs.Contains(p.Item1) && TimeComparator.containsDay(dateContenue, p.Item4, p.Item5)
                                           select p.Item3).Sum());
        ...
        private void initChartData()
        {
            ...
            peData = projetEtapes
                ...
                .ToList();
            List<string> vendeursNoms = vendeurs.Select(v => v.CachedNom).ToList();
            ...
            foreach (Etape etape in EtapesQuery)
            {
                ...
                {
                    weekly[i] = getTotalEtapeWeek(etape.Nom, fridaysBefore[i], vendeursNoms);
                }
        ...
        partial void ShowStats_Execute()
        {
